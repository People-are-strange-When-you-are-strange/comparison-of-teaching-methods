...
#Определение функции gradient_descent () . В этой функции цикл заканчивается, когда либо:
...
#   1.max_iterations : Максимальное количество итераций для запуска
#   2.threshold : Остановка, если разница в значениях функции между двумя последовательными итерациями падает ниже этого порога
#   3.w_init : Начальная точка, с которой начинается градиентный спуск
#   4.obj_func : Ссылка на функцию, которая вычисляет целевую функцию
#   5.grad_func : Ссылка на функцию, вычисляющую градиент функции
#   6.extra_params : Дополнительные параметры (при необходимости) для obj_func и grad_func
#   7.learning_rate : Размер шага для градиентного спуска. Он должен быть в [0,1]
#   8.momentum : Momentum to use. Он должен быть в [0,1]
#Функция вернет:
#   1.w_history : Все точки в пространстве, посещенные градиентным спуском, в которых оценивалась целевая функция
#   2.f_history : Соответствующее значение целевой функции, вычисленное в каждой точке
#
#
#Значения целевой функции
# gradient_descent(5, -1, w_init, f, grad, [], eta, alpha)
# or
#gradient_descent(
#        100, 0.01, w_init,
